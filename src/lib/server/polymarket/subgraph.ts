...
const SUBGRAPH_URLS = {
	...
	markets: "https://api.thegraph.com/subgraphs/name/polymarket/matic-markets-v2",
};
...
export class SubgraphClient {
    ...
    `;
		...
		try {
			...
			return { timestamp: Number.parseInt(result.trades[0].timestamp) * 1000 };
		} catch {
    ...
    `;
		...
		try {
			...
			return result.user ? Number.parseInt(result.user.tradeCount) : 0;
		} catch {
			return 0;
		}
	}

...
export async function fetchAccountData(address: string): Promise<AccountData> {
  ...
  // Get positions
  const positions = (await dataApiClient.getUserPositions(
    address,
  )) as Position[];
  ...
  const closedPositions = positions.filter(
    (p: Position) => p.realizedPnl !== 0,
  );
  const wins = closedPositions.filter(
    (p: Position) => p.realizedPnl > 0,
  ).length;
  const winRate =
    closedPositions.length >= 2 ? wins / closedPositions.length : 0;

  // Calculate profits
  const realizedProfit = positions.reduce(
    (sum: number, p: Position) => sum + (p.realizedPnl || 0),
    0,
  );
  const unrealizedProfit = positions.reduce(
    (sum: number, p: Position) => sum + (p.cashPnl || 0),
    0,
  );
  ...
  for (const trade of trades) {
    if (trade.title?.toLowerCase().includes("president"))
      categories.add("politics");
    if (trade.title?.toLowerCase().includes("election"))
      categories.add("politics");
    if (trade.title?.toLowerCase().includes("trump"))
      categories.add("politics");
    if (trade.title?.toLowerCase().includes("biden"))
      categories.add("politics");
  }
...
export async function fetchWalletPositions(
  ...
  const totalValue = positions.reduce(
    (sum: number, p: Position) => sum + (p.currentValue || 0),
    0,
  );
  ...
  const largestPosition = positions.reduce(
    (max: Position | null, p: Position) =>
      !max || (p.currentValue || 0) > (max.currentValue || 0) ? p : max,
  ...
  return {
    ...
    largestPosition: largestPosition
      ? {
          ...
          pnl:
            (largestPosition.realizedPnl || 0) + (largestPosition.cashPnl || 0),
        }
      : null,
  };
}
...
export async function compareToMarket(
  ...
  const traderLargestTrade = Math.max(
    ...marketTrades.map((t) => t.usdcSize),
    0,
  );
...
export async function executeTool(
  ...
  switch (toolName) {
    ...
    case "compare_to_market":
      return compareToMarket(input.address as string, input.marketId as string);

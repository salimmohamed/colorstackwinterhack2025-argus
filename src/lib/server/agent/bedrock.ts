...
export class BedrockClient {
  ...
  constructor(config: BedrockConfig) {
    ...
    // Use Sonnet 3.5 inference profile (4.5 has tight rate limits)
    this.modelId =
      config.modelId || "us.anthropic.claude-3-5-sonnet-20241022-v2:0";
  }
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (isRetryable) {
          ...
          console.log(
            `[Bedrock] ${error.name || "Error"}, retrying in ${(delay / 1000).toFixed(1)}s (attempt ${attempt + 1}/${maxRetries})`,
          );
 ...
 */
export function isToolUse(block: ContentBlock): block is ContentBlock & {

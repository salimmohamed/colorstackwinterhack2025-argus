...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[Detection] Running detection on ${limitMarkets} markets, min score: ${minRiskScore}`);
    ...
    console.log(`[Detection] Found ${markets.length} markets, analyzing ${marketsToAnalyze.length}`);
    ...
    for (const market of marketsToAnalyze) {
      ...
      if (!conditionId) {
        console.log(`[Detection] No conditionId for market ${market.slug}, skipping`);
        continue;
      }

      try {
        const suspects = await analyzeMarketForInsiders(conditionId, minRiskScore);
        console.log(`[Detection] Found ${suspects.length} suspects in ${market.slug}`);
        ...
        for (const suspect of suspects) {
          try {
            ...
            console.error(`[Detection] Error saving suspect ${suspect.wallet}:`, error);
          }
        }
        ...
        console.error(`[Detection] Error analyzing market ${market.slug}:`, error);
      }
    ...
    const totalSuspects = allResults.reduce((sum, r) => sum + r.suspects.length, 0);
